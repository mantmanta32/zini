...
export class BucketManager {
  ...
  private ensureStatsKeys(): void {
    for (const b of this.customBuckets) {
      if (!this.stats.custom[b.id]) {
        this.stats.custom[b.id] = { id: b.id, name: b.name, icon: b.icon, buyVol: 0, sellVol: 0, count: 0 };
      }
    }
  }
  ...
  processTrade(
    ...
    if (!this.stats.preset[defaultBucket]) {
      this.stats.preset[defaultBucket] = { id: defaultBucket, name: defaultBucket, icon: '🦐', buyVol: 0, sellVol: 0, count: 0 };
    }
    if (isBuyerMaker) {
      this.stats.preset[defaultBucket].sellVol += notionalValue;
    } else {
      this.stats.preset[defaultBucket].buyVol += notionalValue;
    }
    this.stats.preset[defaultBucket].count += 1;
    ...
    if (matchedCustom) {
      ...
      if (!this.stats.custom[matchedCustom.id]) {
        this.stats.custom[matchedCustom.id] = { id: matchedCustom.id, name: matchedCustom.name, icon: matchedCustom.icon, buyVol: 0, sellVol: 0, count: 0 };
      }
      if (isBuyerMaker) {
        this.stats.custom[matchedCustom.id].sellVol += notionalValue;
      } else {
        this.stats.custom[matchedCustom.id].buyVol += notionalValue;
      }
      this.stats.custom[matchedCustom.id].count += 1;
    }

    const finalBucketRef: BucketRef = matchedCustom 
      ? { kind: 'custom', id: matchedCustom.id } 
      : { kind: 'preset', key: defaultBucket };
  ...
  getBucketRollingStats(
    ...
    for (let i = this.rollingTrades.length - 1; i >= 0; i--) {
      ...
      const isMatch = bucketRef.kind === 'preset'
        ? t.bucket.kind === 'preset' && t.bucket.key === bucketRef.key
        : t.bucket.kind === 'custom' && t.bucket.id === bucketRef.id;
    ...
    if (this.hasKlineBootstrap && this.klineHistory.length > 0 && actualSpanMs < windowMs * 0.7) {
      ...
      const weight = bucketRef.kind === 'preset' ? (defaultWeights[bucketRef.key] || 0.05) : 0.05;
  ...
  getAllBucketsSorted(
    ...
    for (const d of defaults) {
      const s = this.stats.preset[d.key] || { buyVol: 0, sellVol: 0, count: 0 };
      const r = this.getBucketRollingStats({ kind: 'preset', key: d.key }, tf, currentTime);
      list.push({
        id: d.key,
        name: d.name,
        icon: d.icon,
    ...
    for (const b of this.customBuckets) {
      ...
      const s = this.stats.custom[b.id] || { buyVol: 0, sellVol: 0, count: 0 };
      const r = this.getBucketRollingStats({ kind: 'custom', id: b.id }, tf, currentTime);
      list.push({
        id: b.id,
        name: b.name,
        icon: b.icon,
  ...
  getSmartMoneyDivergence(
    ...
    const shrimpStats = this.getBucketRollingStats({ kind: 'preset', key: 'shrimp' }, tf, currentTime);
    const whaleStats = this.getBucketRollingStats({ kind: 'preset', key: 'whale' }, tf, currentTime);
    const leviathanStats = this.getBucketRollingStats({ kind: 'preset', key: 'leviathan' }, tf, currentTime);
    ...
    for (const b of this.customBuckets) {
      if (!b.isActive) continue;
      const stats = this.getBucketRollingStats({ kind: 'custom', id: b.id }, tf, currentTime);
      if (b.isSmartMoney) {
